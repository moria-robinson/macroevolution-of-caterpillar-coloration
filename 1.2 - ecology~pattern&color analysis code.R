...
# 1.2 - Phylogenetic logistic regression: Associations between diet ecology (host plant growth form, tissue consumed, diet breadth) and coloration (pattern and colors)
...
for (x in 1:nreps){
  setwd("~/MSdata/bootstraps/ecologyColoration-outputs") # establish a destination folder for bootstrap outputs
  cat(file = "ecologyColoration-runs.txt","i am on", x, append = T)
  tStart<-Sys.time()
  ...
  # Establish which columns are the (1) predictors (color an pattern) and (2) response (diet breadth, growth form, tissue consumed)
  ...
  resp0 <- temp6[,which(names(temp6) %in% c("genus_species", ecology))] 
  pred0 <- temp6[,-which(names(temp6) %in% c(ecology))] 
  ...
  rm(frequencies, frequencies2,  pred0, resp0, temp2, temp3, temp4, temp5, keep, ecology, pattern)
  ...
  for (i in 1:length(respTraits)){
    for (j in 1:length(predTraits)){
      if(respTraits[i]==predTraits[j]){phyloGLM.r[i,j]<-1}
        ...
        fit.temp<-phyloglm(trait1 ~ trait2, phy=pruned.tree, method="logistic_IG10")
  ...
  write.csv(phyloGLM.p3, file = paste0("ecologyColoration_phyloGLM.p-",x,".csv"))
  write.csv(phyloGLM.r3, file = paste0("ecologyColoration_phyloGLM.r-",x,".csv"))
  write.csv(temp9, file = paste0("ecologyColoration_dataBootstraps-",x,".csv"))
  ...
  cat(file = "ecologyColoration-runs.txt", ", hey that took ",print(Sys.time()-tStart), "mins" ,"\r", append = T)
}
